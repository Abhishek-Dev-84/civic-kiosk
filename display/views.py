    ...
    try:
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                messages.error(request, 'System error occurred. Please try again later.')
                return render(request, 'auth.html')
            except Exception as e:
        ...
        logger.error(f"Critical error in auth view: {str(e)}")
...
def otp(request):
    ...
    try:
            ...
            if is_valid:
                ...
                try:
                    ...
                    if existing_session:
                        ...
                        existing_session.user_agent = request.META.get('HTTP_USER_AGENT', '')[:255]
        ...
        logger.error(f"Critical error in OTP view: {str(e)}")
        logger.error(traceback.format_exc())
        messages.error(request, 'System error occurred. Please try again.')
        return redirect('auth')
...
def resend_otp(request):
    ...
    try:
        ...
        logger.error(f"Critical error in resend OTP view: {str(e)}")
        messages.error(request, 'System error occurred. Please try again.')
        return redirect('auth')
...
def menu(request):
    ...
    try:
        ...
        logger.error(f"Error in menu view: {e}")
        messages.error(request, 'Error loading menu. Please try again.')
        return redirect('auth')
...
def electricity_services(request):
    ...
    try:
        ...
        logger.error(f"Error in electricity services: {e}")
        messages.error(request, 'Error loading electricity services. Please try again.')
        return redirect('menu')
    ...
    try:
        if request.method == 'POST':
            ...
            if action == 'pay':
                ...
                # In production, save payment to database
                try:
                    # Get consumer
                    consumer_id = request.session.get('consumer_id')
                    consumer = Consumer.objects.get(id=consumer_id)
                    
                    # Get electricity consumer
                    try:
                        elec_consumer = ElectricityConsumer.objects.get(consumer=consumer)
                        
                        # Create payment record (in production, link to actual bill)
                        payment = ElectricityPayment.objects.create(
                            bill=None,  # Would link to actual bill
                        ...
                        logger.info(f"Payment recorded: {transaction_id}")
                        
                    except ElectricityConsumer.DoesNotExist:
                        logger.warning(f"No electricity consumer found for user {consumer_id}")
                ...
                if len(consumer_number) < 6 or not consumer_number.isdigit():
        ...
        logger.error(f"Error in electricity bill payment: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'electricity-bill-payment.html', context)
...
def electricity_duplicate_bill(request):
    ...
    try:
        if request.method == 'POST':
            ...
            if action == 'download':
                ...
                if bill_id and bill_month:
                    # Log download
                    log_security_event(request, 'BILL_DOWNLOAD', {
                        'bill_id': bill_id,
                        'month': bill_month
                    })
                ...
                if not bill_month or not bill_amount or not bill_number:
                    messages.error(request, 'Invalid bill details')
                    return redirect('electricity-duplicate-bill')
                ...
                # In production, fetch actual bills from database
                today = datetime.now().date()
                bills = []
                
                # Simulate fetching bills (replace with database query)
                for i in range(3):
        ...
        logger.error(f"Error in duplicate bill: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'electricity-duplicate-bill.html', context)
...
def electricity_solar(request):
    ...
    try:
        if request.method == 'POST':
            ...
            if not solar_capacity:
                messages.error(request, 'Please enter solar capacity')
                return render(request, 'electricity-solar.html')
            
            # Validate solar capacity
            try:
                capacity = float(solar_capacity)
                if capacity <= 0:
                    messages.error(request, 'Solar capacity must be positive')
                    return render(request, 'electricity-solar.html')
                if capacity > 100:
                    messages.error(request, 'Maximum solar capacity is 100 kW')
                    return render(request, 'electricity-solar.html')
            except ValueError:
                messages.error(request, 'Please enter a valid solar capacity')
                return render(request, 'electricity-solar.html')
            ...
            # Log application
            log_security_event(request, 'SOLAR_APPLICATION', {
        ...
        logger.error(f"Error in solar application: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'electricity-solar.html')
    ...
    try:
        if request.method == 'POST':
            ...
            if not property_type:
                messages.error(request, 'Please select property type')
                return render(request, 'electricity-new-connection.html')
            ...
            try:
                ...
                # Create electricity consumer (in production)
                # This would be created after approval, not immediately
                
                # Log application
                log_security_event(request, 'NEW_CONNECTION', {
        ...
        logger.error(f"Error in new connection: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'electricity-new-connection.html')
...
def electricity_name_transfer(request):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            # Log request
            log_security_event(request, 'NAME_TRANSFER', {
                'consumer': consumer_number,
                'new_owner': new_owner_name[-4:],
        ...
        logger.error(f"Error in name transfer: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'electricity-name-transfer.html', context)
...
def electricity_meter_replacement(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Validate required fields
            if not meter_type:
                messages.error(request, 'Please select meter type')
                return render(request, 'electricity-meter-replacement.html')
            
            if not reason:
                messages.error(request, 'Please select reason for replacement')
                return render(request, 'electricity-meter-replacement.html')
            
            if not preferred_date:
                messages.error(request, 'Please select preferred date')
                return render(request, 'electricity-meter-replacement.html')
            ...
            # Log request
            log_security_event(request, 'METER_REPLACEMENT', {
                'consumer': consumer_number,
        ...
        logger.error(f"Error in meter replacement: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'electricity-meter-replacement.html')
...
def electricity_load_enhancement(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Log request
            log_security_event(request, 'LOAD_ENHANCEMENT', {
                'consumer': consumer_number,
        ...
        logger.error(f"Error in load enhancement: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'electricity-load-enhancement.html')
...
def electricity_complaint(request):
    ...
    try:
        if request.method == 'POST':
            ...
            if len(complaint_description.strip()) < 10:
            ...
            # Log complaint
            log_security_event(request, 'COMPLAINT_REGISTERED', {
                'consumer': consumer_number[-4:],
                'type': complaint_type,
        ...
        logger.error(f"Error in complaint: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'electricity-complaint.html')
    ...
    try:
        if request.method == 'POST':
            ...
            if action == 'pay':
                ...
                # Validate consumer number
                if len(consumer_no) < 6 or not consumer_no.isdigit():
                    messages.error(request, 'Please enter a valid consumer number')
                    return render(request, 'water-bill.html')
        ...
        logger.error(f"Error in water bill: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'water-bill.html')
...
def municipal_services(request):
    ...
    try:
        ...
        logger.error(f"Error in municipal services: {e}")
        messages.error(request, 'Error loading municipal services.')
        return redirect('menu')
...
def trade_license(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Log application
            log_security_event(request, 'TRADE_LICENSE', {
                'business': business_name[:50],
                'reference': ref_number
            })
        ...
        logger.error(f"Error in trade license: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'trade-license.html')
...
def property_tax(request):
    ...
    try:
        ...
        logger.error(f"Error in property tax: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'property-tax.html')
...
def professional_tax(request):
    ...
    try:
        ...
        logger.error(f"Error in professional tax: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'professional-tax.html')
...
def application_status(request):
    ...
    try:
        ...
        logger.error(f"Error in application status: {e}")
        messages.error(request, 'Error loading status information.')
        return render(request, 'application-status.html', {'application': None, 'complaint': None})
...
def building_plan(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Log application
            log_security_event(request, 'BUILDING_PLAN', {
                'owner': owner_name[:50],
                'reference': ref_number
            })
        ...
        logger.error(f"Error in building plan: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'building-plan.html')
...
def gas_services(request):
    ...
    try:
        ...
        logger.error(f"Error in gas services: {e}")
        messages.error(request, 'Error loading gas services.')
        return redirect('menu')
...
def gas_subsidy(request):
    ...
    try:
        ...
        logger.error(f"Error in gas subsidy: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'gas-subsidy.html')
...
def gas_new_connection(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Log application
            log_security_event(request, 'GAS_NEW_CONNECTION', {
                'name': full_name[:50],
                'reference': ref_number
            })
        ...
        logger.error(f"Error in gas new connection: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'gas-new-connection.html')
...
def gas_cylinder_booking(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Log booking
            log_security_event(request, 'CYLINDER_BOOKING', {
                'type': cylinder_type,
                'reference': ref_number
            })
        ...
        logger.error(f"Error in cylinder booking: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'gas-cylinder-booking.html')
...
def gas_complaint(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Log complaint
            log_security_event(request, 'GAS_COMPLAINT', {
                'consumer': consumer_number[-4:],
                'type': complaint_type,
                'reference': ref_number
            })
        ...
        logger.error(f"Error in gas complaint: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'gas-complaint.html')
...
def gas_bookings_status(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Simulate different booking statuses based on reference number
        ...
        logger.error(f"Error in booking status: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'gas-booking-status.html', context)
...
def gas_bill_payment(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Log payment
            log_security_event(request, 'GAS_PAYMENT', {
                'consumer': consumer_number[-4:],
                'amount': amount,
        ...
        logger.error(f"Error in gas payment: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'gas-bill-payment.html')
...
def payment_history(request):
    ...
    try:
        ...
        return render(request, 'payment-history.html', {'payments': [], 'filter': 'all'})
...
def receipt_print(request):
    ...
    try:
        ...
        logger.error(f"Error in receipt print: {e}")
...
def birth_certificate(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Log application
            log_security_event(request, 'BIRTH_CERTIFICATE', {
                'child': child_name[:50],
                'reference': ref_number
            })
        ...
        logger.error(f"Error in birth certificate: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'birth-certificate.html')
...
def death_certificate(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Validate date of death (not in future)
            from datetime import date
            if date_of_death:
                try:
            ...
            # Log application
            log_security_event(request, 'DEATH_CERTIFICATE', {
                'deceased': deceased_name[:50],
                'reference': ref_number
            })
        ...
        logger.error(f"Error in death certificate: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'death-certificate.html')
...
def marriage_registration(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Log application
            log_security_event(request, 'MARRIAGE_REGISTRATION', {
                'groom': groom_name[:50],
                'bride': bride_name[:50],
                'reference': ref_number
            })
        ...
        logger.error(f"Error in marriage registration: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'marriage-registration.html')
...
def document_upload_qr_view(request):
    ...
    try:
        ...
        logger.error(f"Error in QR upload: {e}")
        messages.error(request, 'An error occurred. Please try again.')
        return redirect('menu')
...
def document_upload_pen_view(request):
    ...
    try:
        ...
        logger.error(f"Error in pen upload: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'document-upload-pen.html')
...
def document_upload_camera_view(request):
    ...
    try:
        ...
        logger.error(f"Error in camera upload: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'document-upload-camera.html')
...
def grievance(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # Log grievance
            log_security_event(request, 'GRIEVANCE', {
        ...
        logger.error(f"Error in grievance: {e}")
        logger.error(traceback.format_exc())
        messages.error(request, 'An error occurred. Please try again.')
        return render(request, 'grievance.html')
